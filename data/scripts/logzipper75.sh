...
###  Create Date 					: 2016-12-21-Wednesday 
...
###  Script File Name 				: logzipper75.sh			
###  Script Log File Name 			: logzipper75.log
###  Script Log Backup File Name	 : logzipper75.log_old
###  Script Path      				: /app/scripts/                  
###  Last updated date				: 2016-12-29
...
#================================================================================================================================#
		...
		export script_file_name="logzipper75.sh"
		export script_log_file="logzipper75.log"
		export script_log_old_file="logzipper75.log_old"
		...
		export logzipper75_lock_file="logzipper75.lock"
...
#================================================================================================================================#
	...
	{
		cd /app/scripts
		## Check if logzipper75 is already running
		if [ -f "$script_location/$logzipper75_lock_file" ] ;
			then
				PID=$(cat $script_location/$logzipper75_lock_file)
				ps -p $PID > /dev/null 2>&1
					if [ $? -eq 0 ];
					then
						echo -e "[ $action_date_time ] \t logzipper75 is already running ... Hence exiting ..." >> $script_log_file_path
						exit 1
					else
						## Process not found -- assumed "logzipper75" script not running
						sudo -u admweb chmod 666 $script_location/$logzipper75_lock_file
						echo $$ > $script_location/$logzipper75_lock_file
							if [ $? -ne 0 ]
							then
								echo -e "[ $action_date_time ] \t Could not start new process of logzipper75 ... Hence exiting ..." >> $script_log_file_path
								exit 1
							fi
					fi	
		else
		## LOCK file not found -- assumed "logzipper75" script not running
			sudo -u admweb touch $script_location/$logzipper75_lock_file
			sudo -u admweb chmod 666 $script_location/$logzipper75_lock_file
			echo $$ > $script_location/$logzipper75_lock_file
				if [ $? -ne 0 ]
				then
					echo -e "[ $action_date_time ] \t Could not start logzipper75 ... Hence exiting ....	" >> $script_log_file_path
					exit 1
				fi
		fi
		...
		if [ `ps -ef | grep -i "rotatelog" | grep -v grep | grep -v "apache" | wc -l` -gt 1 ]
			then
				echo -e "[ $action_date_time ] \t Could not start logzipper75 ... Since \"rotatelog\" is already running .... Hence ... exiting ...	" >> $script_log_file_path
				### correction needed here
		fi
		
		sudo -u admweb chmod 444 $script_location/$logzipper75_lock_file
	}
	...
	function clean_up2
	{
		...
		sudo -u admweb chmod 777 $script_location/$logzipper75_lock_file
		sudo -u admweb rm -f $script_location/$logzipper75_lock_file
	...
	{	
		if [ -a "$script_location/$script_log_file" ];
			then
				...
				script_log_file_path="$script_location/logzipper75.log"
...
#================================================================================================================================#
	function check_for_parameters2
	{
		...
		then
		   ...
		   echo -e "[ $action_date_time ] \t logzipper75 was invoked with less permission. Use sudo -u admweb to invoke the script" >> $script_log_file_path
		...
		then
			echo -e "[ $action_date_time ] \t logzipper75 was called but necessary parameters were missing. Script needs to be called by passing 2 parameters - 1st) the prescribed threshold df value of \"/logs\" and 2nd) the minimum file retention period for \"/logs\" in number of days" >> $script_log_file_path
			echo "########################################################################################################################" >> $script_log_file_path
			clean_up2
			exit 1
		fi
...
#================================================================================================================================#
	function start_and_end_logzipper75
	{
		...
		then
			echo " "
			echo -e "[ $action_date_time ] \t Size of \"/logs\" - $capture_scan_location_usage_value%. Hence \"rotatelog\" was manually invoked" >> $script_log_file_path # new line added formatted with tab
